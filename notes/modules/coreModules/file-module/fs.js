...
fs.writeFileSync('./notes/modules/coreModules/output/syncFile.txt', 'hi hemanth');

// creates the async file, whenever user changes the data will be updated sync way  
fs.writeFile('./notes/modules/coreModules/output/asyncFile.txt', 'async data have created',(err)=>{
    if(err){
...
const readMe = fs.readFileSync('./notes/modules/coreModules/output/readFile.txt', 'utf-8');
...
fs.readFile('./notes/modules/coreModules/output/readFile.txt', 'utf-8', (err, res)=>{
    if(err){
